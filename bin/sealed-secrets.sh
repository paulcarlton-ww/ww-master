...
function usage()
{
    ...
    echo "<key-dir> is the path of the local directory to store keys in, directory will be created if not present"
    echo "Creates public and private keys and deploys private key to cluster"
}
...
function args() {
  ...
  while (( arg_index < arg_count )); do
    case "${arg_list[${arg_index}]}" in
          "--key-dir") (( arg_index+=1 ));key-dir="${arg_list[${arg_index}]}";;
...
${repo_dir}/bin/setup-kubeseal.sh ${debug} --privatekey-file ${keys_dir}/sealed-secrets-key --pubkey-file ${keys_dir}/sealed-secrets-cert.crt